...
;(function (window, undefined) {
  ...
  window.corOnload = function () {
    ...
    window.corNative = Object.assign(window.corNative || {}, {
      ...
      //从界面底部弹出按钮列表
      actionSheetDlg: function (parmas, callBack) {
      ...
      requestAjax: function (params, callBack) {
        // var notTest = !~params.url.indexOf('delete_collection');
  ...
  function initCorNative() {
    ...
    window.corNative = {
      ...
      confirmDlg:function(params,callBack){
        vm.$messagebox.confirm(params.message, params.title).then(function(){
          callBack(1);
        });
      },
      promptDlg:function(params,callBack){
        vm.$messagebox.prompt(' ', params.message).then(function(obj){
          if(obj.value){
            callBack(1,obj.value);
          }
        })
      },
      toastDlg:function(params){
        if(params.type==0){
          var loc="";
          ...
          vm.$indicator.open({text:params.msg,spinnerType: 'double-bounce' });
        }
      },
      closeToastDlg:function(){
        vm.$indicator.close();
      },
      actionSheetDlg:function(params,callBack){
        var newDialog = document.createElement("div");
        newDialog.id="actionMneu";
        newDialog.className = "co-flex co-ver co-je";
        newDialog.style.width = "100%";
        newDialog.style.height = "100%";
        newDialog.style.background = "rgba(0,0,0,.5)";
        newDialog.style.position = "absolute";
        newDialog.style.zIndex  = "99";
        var html1 = '<div class="co-tx-c" style="height:2.2rem;line-height:2.2rem;background:#fff;color:#000;border-bottom:1px #ccc solid">'+params.title+'</div>';
        var html2 = "";
        for(var i=0;i<params.buttons.split(",").length;i++){
          html2+='<div class="co-tx-c" style="height:2.2rem;line-height:2.2rem;background:#fff;color:#000;border-bottom:1px #ccc solid">'+params.buttons.split(",")[i]+'</div>'
        }
        var html3 = '<div class="co-tx-c"   style="height:2.2rem;line-height:2.2rem;background:#eee;color:#000;border-bottom:1px #ccc solid">'+params.cancel+'</div>';
        newDialog.innerHTML = html1 + html2 + html3;
        document.body.insertBefore(newDialog,document.body.children[0]);
        var actionMneuList = document.getElementById("actionMneu").children;
        for(var i=0;i<actionMneuList.length;i++){
          (function(arg){     
            actionMneuList[i].onclick = function() {     
             callBack(arg-1);   
             document.getElementById("actionMneu").parentNode.removeChild(document.getElementById("actionMneu"));
            };   
          })(i)  
        }
      },
  ...
  setTimeout(function () {
    if (window.corXmlHttpMgr === undefined) {
      !window.isPC && initCorNative();
      window.isPC = true;        
    }    
  }, 250);
